...
use app\back\validate\WeigthUnitValidate;
use app\back\model\WeigthUnit;

use think\Controller;
use think\Db;
use think\Session;

class WeigthUnitController extends Controller
{
    ...
    public function setAction(){
        ...
        if($request->isGet()){
            if(Session::get('message')==''&&Session::get('data')==''){
                ...
                $data = [];
                if(!empty($id))
                    $data = Db::name('weigth_unit')->find($id);  //查询出来数据
            }
            else{
        ...
        elseif ($request->isPost()){
            ...
            $validate = new WeigthUnitValidate;
